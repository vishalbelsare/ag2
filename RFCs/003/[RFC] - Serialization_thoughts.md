...
async def run_agents_and_process_events():
    ...
    )] # Save the messages of the chat so that it can be replayed?

    chat = Chat(alice, bob, callbacks=callbacks)

    response = await a_run(chat, message="Who is Leonardo da Vinci?")

    # Save the chat as a json of agent configurations?
    # Like keras: Chat == Model, Agent == Layer, chat_history/messages == weights, or agent_states == weights?, do we need messages?

    return response

async def load_run_response():
    chat = Chat(alice, bob, callbacks=callbacks) # Is it viable to define another chat with another actors and load the previous chat history into it?

    chat.load_checkpoints(f"/checkpoints/alice-and-bob/{datetime.now()}") # Load checkpoints is like load weights

    response = await a_run(chat, message="Resume the conversation") # Or even no message defined
