...
To enable batch grading, set `batch_execution=True` in the
`reason_config`. By default, `batch_execution` is set to `False`,
meaning individual node grading is performed without batching.
