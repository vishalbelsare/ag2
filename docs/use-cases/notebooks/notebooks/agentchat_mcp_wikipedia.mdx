---
custom_edit_url: https://github.com/ag2ai/ag2/edit/main/notebook/agentchat_mcp_wikipedia.ipynb
description: 'Conversational Workflows with MCP: A French joke on a random Wikipedia
  article'
source_notebook: /notebook/agentchat_mcp_wikipedia.ipynb
tags:
- MCP
title: 'Conversational Workflows with MCP: A French joke on a random Wikipedia article'
---

<a href="https://colab.research.google.com/github/ag2ai/ag2/blob/main/notebook/agentchat_mcp_wikipedia.ipynb" class="colab-badge" target="_blank"><img noZoom src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab" /></a>
<a href="https://github.com/ag2ai/ag2/blob/main/notebook/agentchat_mcp_wikipedia.ipynb" class="github-badge" target="_blank"><img noZoom src="https://img.shields.io/badge/Open%20on%20GitHub-grey?logo=github" alt="Open on GitHub" /></a>
...
-   Imports from the `autogen` and `mcp` libraries, which are used to
    create conversational agents and connect to an wikipedia-related
    tool server.
...
tool execution related to wikipedia article retrieval.


```python
# Path to the arxiv MCP server
mcp_server_path = Path("mcp/mcp_wikipedia.py")
```

## Agents

-   Define the **joker agent**, whose role is to make jokes in French.
...
-   Define a `ContextVariables` object to inject context (like joke
    constraints) into the agent’s workflow.
...
class JokeResponse(BaseModel):
    joke_instructions: str = Field(..., description="instruction, not in French")
    joke: str = Field(..., description="joke in French")
    joke_explanation: str = Field(..., description="explanation, not in French")
...
server_params = StdioServerParameters(
    command="python", args=[str(mcp_server_path), "stdio", "--storage-path", "mcp/wikipedia_articles"]
)
...
<div className="edit-url-container">
    <a className="edit-url" href="https://github.com/ag2ai/ag2/edit/main/notebook/agentchat_mcp_wikipedia.ipynb" target='_blank'><Icon icon="pen" iconType="solid" size="13px"/> Edit this page</a>
</div>
