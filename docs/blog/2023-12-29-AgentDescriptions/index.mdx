...
AutoGen 0.2.2 introduces a [description](https://docs.ag2.ai/reference/autogen/ConversableAgent#conversableagent) field to ConversableAgent (and all subclasses), and changes GroupChat so that it uses agent `description`s rather than `system_message`s when choosing which agents should speak next.
