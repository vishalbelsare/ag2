...
def __init__(*,
             ...
             func_or_tool: Union["Tool", Callable[..., Any]]) -> None
```
...
- `func_or_tool` _Union[Tool, Callable[..., Any]]_ - The function or Tool instance to create a Tool from.
...
```python
def register_tool(agent: "ConversableAgent") -> None
```
...
```python
@property
def realtime_tool_schema() -> dict[str, Any]
```
