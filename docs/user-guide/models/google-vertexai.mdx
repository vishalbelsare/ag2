...
llm_config_gemini = LLMConfig(path="OAI_CONFIG_LIST").where(model=["gemini-1.5-pro"])
...
image_agent = MultimodalConversableAgent(
    "Gemini Vision",
    llm_config=LLMConfig(config_list=gemini_vision_config, seed=seed),
...
assistant = AssistantAgent("assistant", llm_config=LLMConfig(config_list=openai_gemini_config), max_consecutive_auto_reply=3)
