...
| Type | Description |
|--|--|
| `autogen.cache.cache.Cache` | Cache: A Cache instance configured for Cosmos DB. |
...
```python
disk(cache_seed: Union[str, int] = 42, cache_path_root: str = '.cache') -> autogen.cache.cache.Cache
```
...
```python
redis(cache_seed: Union[str, int] = 42, redis_url: str = 'redis://localhost:6379/0') -> autogen.cache.cache.Cache
```
