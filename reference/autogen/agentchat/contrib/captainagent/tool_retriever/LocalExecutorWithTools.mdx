...
```python
LocalExecutorWithTools(tools: list[autogen.tools.Tool] | None = None, work_dir: pathlib.Path | str = PosixPath('.'))
```
...
| Name | Description |
|--|--|
| `tools` | The tools to inject into the code execution environment.<br/><br/>Default is an empty list.<br/><br/>**Type:** `list[autogen.tools.Tool] \| None`<br/><br/>**Default:** None |
...
```python
execute_code_blocks(self, code_blocks: list[autogen.coding.CodeBlock]) -> autogen.coding.CodeResult
```
