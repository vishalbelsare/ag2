        ...
        Args:
            ...
            reason_config (Optional[dict[str, Any]]): Configuration for the reasoning method.
                ...
                Common parameters:
                    ...
                    async_mode (bool): Whether to use async parallel LLM calls for faster searching (default: False)
                ...
                Example configs:
                    `{"method": "beam_search", "beam_size": 5, "max_depth": 4, "async_mode": True}`
                    `{"method": "mcts", "nsim": 10, "exploration_constant": 2.0, "async_mode": True}`
                    `{"method": "lats", "nsim": 5, "forest_size": 3, "async_mode": True}`
...
""",
    ...
    async def a_generate_forest_response(
        self,
        messages: Optional[list[dict[str, Any]]] = None,
        sender: Optional[Agent] = None,
        config: Optional[dict[str, Any]] = None,
    ) -> tuple[bool, str]:
        """Async version of generate_forest_response with parallel forest generation.
        ...
        # Generate forest answers in parallel
        forest_tasks = []
        for _ in range(self._forest_size):
            if self._method in ["beam_search", "dfs"]:
                forest_tasks.append(self.a_beam_reply(prompt, ground_truth))
            elif self._method in ["mcts", "lats"]:
                forest_tasks.append(self.a_mcts_reply(prompt, ground_truth))
            else:
                raise ValueError("Invalid reasoning method specified.")

        forest_answers = await asyncio.gather(*forest_tasks)
...
"""
    ...
    async def a_rate_node(self, node: ThinkNode, ground_truth: Optional[str] = None, is_outcome: bool = False) -> float:
        """Async version of rate_node for parallel rating operations.
...
"""
    ...
    async def a_execute_node(self, node: ThinkNode) -> Optional[str]:
        """Async version of execute_node for parallel execution operations.
...
"""
    ...
    async def a_beam_reply(self, prompt: str, ground_truth: Optional[str] = None) -> str:
        """Async version of _beam_reply with parallel LLM calls for faster searching.

        Implements beam search through a tree of reasoning steps, using parallel
        expansion, rating, and execution operations to speed up the search process.
        ...
        while prev_leafs and len(final_answers) < self._beam_size:
            ...
            if len(prev_leafs) + len(final_answers) > self._beam_size:
                ...
                if self._batch_grading:
                    ...
                    # Rate all nodes in parallel
                    rating_tasks = [self.a_rate_node(node, ground_truth) for node in prev_leafs]
                    ratings = await asyncio.gather(*rating_tasks)
                ...
                # Parallel execution
                if self._interim_execution:
                    execution_tasks = [self.a_execute_node(node) for node in prev_leafs]
                    outputs = await asyncio.gather(*execution_tasks)
                    for node, output in zip(prev_leafs, outputs):
                        node.output = output
...
"""
    ...
    def _mcts_reply(self, prompt: str, ground_truth: Optional[str] = None) -> str:
        ...
        # Note: For parallel execution, use async_mode=True in reason_config
...
""",
    ...
    async def a_mcts_reply(self, prompt: str, ground_truth: Optional[str] = None) -> str:
        """Async version of _mcts_reply with parallel MCTS simulations for faster searching.
        ...
        simulation_tasks = [self._a_single_mcts_simulation(root, prompt, ground_truth) for _ in range(self._nsim)]
        answer_nodes = await asyncio.gather(*simulation_tasks)

        # Flatten the list and update LATS context
        all_answer_nodes = []
    ...
    async def _a_single_mcts_simulation(
        ...
        Args:
            root (ThinkNode): The root node of the search tree.
            prompt (str): The original prompt.
            ground_truth (Optional[str]): Ground truth for evaluation.

        Returns:
            Optional[ThinkNode]: The answer node from this simulation.
        """
        node = root
        ...
        while not self._is_terminal(node) and len(node.children) > 0:
            ...
            # Execution
            if self._interim_execution:
                node.output = await self.a_execute_node(node)

        # Expansion and Simulation
        while not self._is_terminal(node):
            if len(node.children) == 0:
                await self.a_expand(node)
            if len(node.children) == 0:
                node.content += "\nTERMINATE"
                break
            node = random.choice(node.children)
...
""",
        ...
        _answer: str = last_msg["content"].strip() if last_msg is not None else ""
        _ans_node = ThinkNode(content=_answer, parent=node)
        reward = await self.a_rate_node(_ans_node, ground_truth, is_outcome=True)
        _ans_node.value = reward
        node.backpropagate(reward)
    ...
    async def a_expand(self, node: ThinkNode) -> list[ThinkNode]:
        """Async version of _expand for parallel expansion operations.
